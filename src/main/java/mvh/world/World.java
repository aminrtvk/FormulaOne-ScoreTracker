...
public class World {
    ...
    public void deleteEntity(int row, int column){
        this.world[row][column] = null;
    }
    ...
    /**
     *
     * @return the String version of the world given by the user
     *
     * we have defined a string builder and depending on the conditionals we append a Symbol
     */
    public String worldString() {
        StringBuilder world = new StringBuilder();
        for (int i = -1; i < this.getRows() + 1; i++) { // we use the number of rows from our world object
            for (int j = -1; j < this.getColumns() + 1; j++) { // we use the number of column from our world object
                if (i >= 0 && i < this.getRows() && j >= 0 && j < this.getColumns()) {
                    if (this.world[i][j] == null) {
                        world.append(String.format("%s\t",Symbol.FLOOR.getSymbol()));
                    } else if (this.isHero(i, j)) {
                        Entity hero = getEntity(i, j);
                        if (hero.isAlive()) {
                            world.append(String.format("%s\t",hero.getSymbol()));
                        } else {
                            world.append(String.format("%s\t",Symbol.DEAD.getSymbol()));
                        }
                    } else if (this.isMonster(i, j)) {
                        Entity monster = getEntity(i, j);
                        if (monster.isAlive()) {
                            world.append(String.format("%s\t",monster.getSymbol()));
                        } else {
                            world.append(String.format("%s\t",Symbol.DEAD.getSymbol()));
                        }
                    }
                    ...
                    world.append(String.format("%s\t",Symbol.WALL.getSymbol()));
                }
            }
            world.append("\n");
        }
